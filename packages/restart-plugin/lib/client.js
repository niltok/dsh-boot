window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
[data-dsh-boot="restart-row"]{box-sizing:border-box;display:flex;width:100%;align-items:center;justify-content:space-between;gap:16px;padding:16px 0;border-bottom:1px solid var(--dsw-alias-divider, rgba(127,127,127,.22))}
[data-dsh-boot="restart-row"]>[data-dsh-boot="text"]{min-width:0;flex:1 1 auto}
...
[data-dsh-boot="restart-button"]{box-sizing:border-box;flex:none;cursor:pointer;height:32px;border-radius:10px;border:1px solid var(--dsw-alias-border, rgba(127,127,127,.35));background:transparent;color:var(--dsw-alias-label-primary, inherit);padding:0 14px;font-family:inherit;font-size:13px;line-height:18px}
[data-dsh-boot="restart-button"]:hover{background:var(--dsw-alias-interactive-bg-hover, rgba(127,127,127,.12))}
[data-dsh-boot="restart-button"]:disabled{cursor:default;opacity:.6}
...
`;
    ...
    const zh = {
      ...
      restart: "重启服务",
      restarting: "正在重启…",
      unavailable: "此 dsh 不是由 dsh-boot 启动的，重启入口不可用。",
      failed: "重启请求失败",
    };
    const en = {
      ...
      unavailable: "This dsh instance was not started by dsh-boot, so restart is unavailable.",
